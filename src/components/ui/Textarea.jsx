  ...
  return (
    <div className={`space-y-1.5 ${wrapperClassName}`.trim()}>
      ...
      <textarea
        ...
        className={`w-full resize-y rounded-md border border-[var(--line)] bg-[var(--surface-elevated)] px-4 py-3 text-[var(--text-main)] outline-none transition-colors duration-150 placeholder:text-[var(--muted)] focus:border-[var(--accent)] focus:bg-[var(--surface-muted)] focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--surface)] ${error ? "border-[color:var(--danger)] focus:border-[color:var(--danger)]" : ""} ${className}`.trim()}
        {...props}
      />
