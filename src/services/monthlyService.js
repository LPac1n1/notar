...
export async function listMonthlySummaries({
  ...
  donationStartDate = "all",
  donorActiveStatus = "active",
  // Optional client-driven pagination. Applied AFTER post-processing
  // (filters, adjustment merging, sort) because the merge synthesizes rows
  // for active donors that don't appear in monthly_donor_summary; pushing the
  // LIMIT into SQL would skip those. Keeps backwards compatibility — when
  // omitted the lister returns every row.
  limit,
  offset = 0,
} = {}) {
 ...
 * (combine with `listMonthlySummaries({ ...filters, limit, offset })`). Note
 * that the count includes synthesized rows for active donors without any
 * matching `monthly_donor_summary` row, so it stays consistent with the
 * client-side count of `listMonthlySummaries`.
 */
